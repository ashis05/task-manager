...
function SignUp() {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        setIsLoading(true);
        try{
            // Assuming your backend has a /signup or /register endpoint
            const response = await axios.post('/api/signup', {name, email, password, confirmPassword});
            
            // THIS IS THE KEY: Saving the token here works exactly like Login
            ...
            const errorMessage = error.response?.data?.message || error.response?.data?.error || error.response?.data?.msg || "Something went wrong";
    ...
    return (
        <div className="flex items-center justify-center h-screen w-full ">
            ...
            <Card className="w-100 h-auto py-4 bg-[#1a2230] border-0 border-[var(--border-dark)] rounded-xl">
                ...
                <CardContent className="text-white flex flex-col items-start justify-center gap-4 mx-5 mt-[-1rem]">
                    ...
                    <form className="flex flex-col gap-6 w-full" onSubmit={handleSubmit}>
                        ...
                        <div className="flex flex-col gap-2">
                            <Label className="font-manrope">Confirm Password</Label>
                            <div className="relative">
                                <Input
                                    id="confirmPassword"
